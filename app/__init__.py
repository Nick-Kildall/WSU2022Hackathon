...
# (milestone 3) import LoginManager and Moment extensions here
from flask_login import LoginManager
from flask_moment import Moment

db = SQLAlchemy()
# (milestone 3) create LoginManager object and configure the login view as 'auth.login', i.e, `login` route in `auth` Blueprint. 
login = LoginManager()
...
def create_app(config_class=Config):
    ...
    db.init_app(app)
    # (milestone 3) Configure the app object for login using `init_app` function. 
    login.init_app(app)
